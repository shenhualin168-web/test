...
#	Author:	Coffee Zhang (Modified)
#	Dscription: Socks5 Installation (Auto Install)
#	Version: 1.2 - Fixed Public IP Output
#====================================================
...
for ((i = 0;i < ${#ips[@]}; i++)); do
    # 获取当前出口的公网IP
    current_public_ip=$(curl --interface ${ips[i]} -s http://ip.sb)
    public_ips+=("$current_public_ip")
    echo -e "${OK} ${GreenBG} 出口IP ${ips[i]} 对应的公网IP: ${current_public_ip} ${Font}"
...
for ((i = 0;i < ${#public_ips[@]}; i++)); do
    echo -e "${public_ips[i]}\t${Port}\t${ss_user}\t${ss_pass}" >> /root/${Port}.txt
done
